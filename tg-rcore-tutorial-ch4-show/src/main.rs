...
extern "C" fn rust_main() -> ! {
    ...
    lec5_lab4::emit_kernel_space_created(
        ks.root_ppn().val(),
        (8usize << 60) | ks.root_ppn().val(),
    );
    ...
    let app_count = unsafe { PROCESSES.get_mut().len() };
    lec5_lab4::emit_init_observables(app_count, MEMORY);
...
extern "C" fn schedule() -> ! {
    ...
    let mut first_enter = true;

    while !unsafe { PROCESSES.get_mut().is_empty() } {
        let proc = unsafe { &mut PROCESSES.get_mut()[0] };

        if first_enter {
            lec5_lab4::emit_portal_enter_user(0, proc.sepc(), proc.satp());
            first_enter = false;
        }
        ...
        match scause::read().cause() {
            scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                ...
                let (id, args, syscall_raw, a0_raw, sepc_raw) = {
                    let proc = unsafe { &PROCESSES.get_mut()[0] };
                    let syscall_raw = proc.syscall_id_raw();
                    let a0_raw = proc.a0();
                    let sepc_raw = proc.sepc();
                    let ctx = &proc.context.context;
                    let id: Id = ctx.a(7).into();
                    let args = [ctx.a(0), ctx.a(1), ctx.a(2), ctx.a(3), ctx.a(4), ctx.a(5)];
                    (id, args, syscall_raw, a0_raw, sepc_raw)
                };

                lec5_lab4::emit_syscall_trap(0, syscall_raw, a0_raw, sepc_raw);
                ...
                match tg_syscall::handle(Caller { entity: 0, flow: 0 }, id, args) {
                    Ret::Done(ret) => match id {
                        Id::EXIT => {
                            ...
                            let remain = unsafe { PROCESSES.get_mut().len() };
                            lec5_lab4::emit_process_exit(0, exit_code, remain);
                            if remain > 0 {
                                let next = unsafe { &PROCESSES.get_mut()[0] };
                                lec5_lab4::emit_process_switch(0, 0, next.satp());
                                first_enter = true;
                            }
                        }
                        _ => {
                            let ctx = &mut unsafe { &mut PROCESSES.get_mut()[0] }.context.context;
            ...
            scause::Trap::Interrupt(scause::Interrupt::SupervisorTimer) => {
                lec5_lab4::emit_timer_interrupt(0);
            }
            e => {
                let sepc = unsafe { PROCESSES.get_mut()[0].sepc() };
                let cause_str = format_exception_str(e);
                lec5_lab4::emit_exception_kill(0, &cause_str, stval::read(), sepc);
                log::error!(
...
fn format_exception_str(trap: scause::Trap) -> alloc::string::String {
    alloc::format!("{:?}", trap)
}
...
/// panic 处理函数：打印 backtrace 后以异常状态关机。
#[panic_handler]
