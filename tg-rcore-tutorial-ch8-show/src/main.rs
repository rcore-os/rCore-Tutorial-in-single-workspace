...
/// lec11 / lec12 知识点串口输出
mod lec11_lec12;
/// 编译期嵌入符号，供运行时解析 `ra` → 函数名/行号/参数
mod symtab_resolve;
/// 帧指针回溯（需 `-C force-frame-pointers=yes`）
mod stackwalk;
...
extern "C" fn rust_main() -> ! {
    ...
    lec11_lec12::emit_init_observables(MEMORY);
    // 步骤 6：异界传送门初始化
    ...
    if let Some((process, thread)) = Process::from_elf(ElfFile::new(initproc.as_slice()).unwrap()) {
        ...
        lec11_lec12::emit_initproc_main_thread(pid.get_usize(), tid.get_usize());
    }
    ...
    loop {
        ...
        if let Some(task) = unsafe { (*processor).find_next() } {
            let run_tid = task.tid.get_usize();
            lec11_lec12::emit_scheduler_pick(run_tid);
            ...
            match scause::read().cause() {
                ...
                scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                    ...
                    let nr = ctx.a(7);
                    lec11_lec12::emit_user_ecall(run_tid, nr, args[0], args[1]);
                    ...
                    match current_proc.signal.handle_signals(ctx) {
                        ...
                        _ => match syscall_ret {
                            Ret::Done(ret) => {
                                lec11_lec12::emit_syscall_result(run_tid, nr, ret);
                                match id {
                                ...
                                Id::SEMAPHORE_DOWN | Id::MUTEX_LOCK | Id::CONDVAR_WAIT => {
                                    ...
                                    *ctx.a_mut(0) = ret as _;
                                    if ret == -1 {
                                        lec11_lec12::emit_thread_blocked(run_tid, nr);
                                        // 阻塞：从就绪队列移除，等待资源释放后唤醒
                                        unsafe { (*processor).make_current_blocked() };
                                    } else {
                                        lec11_lec12::emit_thread_suspend(run_tid);
                                        // 成功获取：正常挂起（时间片轮转）
    ...
    impl tg_syscall::Thread for SyscallContext {
        ...
        fn thread_create(&self, _caller: Caller, entry: usize, arg: usize) -> isize {
            ...
            *context.a_mut(0) = arg;
            ...
            lec11_lec12::emit_thread_created(
                tid.get_usize(),
    ...
    impl SyncMutex for SyscallContext {
        ...
        fn semaphore_up(&self, _caller: Caller, sem_id: usize) -> isize {
            ...
            if let Some(tid) = sem.up() {
                lec11_lec12::emit_resource_wake(b"semaphore_up", sem_id, tid.get_usize());
        ...
        fn mutex_unlock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            if let Some(tid) = mutex.unlock() {
                lec11_lec12::emit_resource_wake(b"mutex_unlock", mutex_id, tid.get_usize());
                unsafe { (*processor).re_enque(tid); }
            }
            0
        }
        ...
        fn condvar_signal(&self, _caller: Caller, condvar_id: usize) -> isize {
            ...
            if let Some(tid) = condvar.signal() {
                lec11_lec12::emit_resource_wake(b"condvar_signal", condvar_id, tid.get_usize());
                unsafe { (*processor).re_enque(tid); }
            }
            0
        }
        ...
        fn condvar_wait(&self, _caller: Caller, condvar_id: usize, mutex_id: usize) -> isize {
            ...
            if let Some(waking_tid) = waking_tid {
                lec11_lec12::emit_resource_wake(
                    b"condvar_wait_wakeup",
                    condvar_id,
