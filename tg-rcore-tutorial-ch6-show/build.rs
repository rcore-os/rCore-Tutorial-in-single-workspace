...
fn main() {
    ...
    // 只在 RISC-V64 架构上使用链接脚本 / 打包 fs / 嵌入符号表
    if target_arch == "riscv64" {
        ...
        // 主机 `cargo check` 等：供 lec9 模块中 env! 占位
        println!("cargo:rustc-env=LAB_M_BASE=host");
        println!("cargo:rustc-env=LAB_S_BASE=host");
        println!("cargo:rustc-env=LAB_TARGET_TRIPLE=host");
    }
}
...
// ===== ELF / DWARF symbol extraction（供 symtab_resolve / 带参 backtrace）=====
...
fn extract_func_symbols(data: &[u8]) -> Vec<(u64, u64, String)> {
    ...
    for i in 0..n {
        ...
        if st_name == 0 || st_value < TEXT_BASE || st_value >= TEXT_LIMIT {
            continue;
        }
