...
//!
//! ## tg-rcore-tutorial-ch6-show 扩展
//!
//! - **lec9**：`lec9_lab6.rs` 中 `[LEC9-LAB6]` 串口锚点，对齐第九讲「文件系统」。
//! - **backtrace**：`stackwalk` + `symtab_resolve`（需 `-C force-frame-pointers=yes`）；用户态异常时打印内核栈；
//!   可选功能 `show-demos` 增加「正常栈」与故意 `panic` 演示。
//! - **GDB**：`scripts/launch-qemu-gdb.sh`、`gdb/ch6.gdb`、`scripts/gdb_ch6_fs_tour.py`。

// 不使用标准库，裸机环境没有操作系统提供系统调用支持
#![no_std]
// 不使用默认的 main 函数入口，裸机环境需要自定义入口点
#![no_main]
// RISC-V 上严格警告；show 版允许部分内部模块无逐条文档
...
mod symtab_resolve {
    ...
    pub fn init() {}
    /// 占位
    pub fn print_fn_for_ra(_ra: usize, _fp: usize) {}
}
#[cfg(target_arch = "riscv64")]
mod stackwalk;
#[cfg(not(target_arch = "riscv64"))]
...
extern "C" fn rust_main() -> ! {
    ...
    lec9_lab6::emit_kernel_space_created(root_ppn, satp_val);
    // 步骤 6：初始化异界传送门
    ...
    let initproc_len = initproc.len();
    lec9_lab6::emit_initproc_loaded(initproc_len);
    if let Some(process) = Process::from_elf(ElfFile::new(initproc.as_slice()).unwrap()) {
        let pid = process.pid.get_usize();
        let entry = process.context.context.pc();
        let user_satp = process.context.satp;
        let heap_bottom = process.heap_bottom;
        lec9_lab6::emit_process_created(pid, entry, user_satp, heap_bottom);
    ...
    lec9_lab6::emit_init_observables(MEMORY, initproc_len, MMIO[0].0, root_ppn);

    #[cfg(feature = "show-demos")]
    {
        println!("\n===== Normal Backtrace Demo =====");
        bt_depth1();
        println!("===== End Normal Backtrace Demo =====\n");
    }

    let mut first_enter_user = true;
    ...
    loop {
        ...
        if let Some(task) = unsafe { (*processor).find_next() } {
            if first_enter_user {
                lec9_lab6::emit_portal_enter_user(
                    task.pid.get_usize(),
                    task.context.context.pc(),
                    task.context.satp,
                );
                first_enter_user = false;
            }
            ...
            match scause::read().cause() {
                ...
                scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                    ...
                    let sepc_at_ecall = ctx.pc();
                    let syscall_raw = ctx.a(7);
                    let id: Id = syscall_raw.into();
                    let args = [ctx.a(0), ctx.a(1), ctx.a(2), ctx.a(3), ctx.a(4), ctx.a(5)];
                    lec9_lab6::emit_syscall_trap(
                        task.pid.get_usize(),
                        syscall_raw,
                        args[0],
                        sepc_at_ecall,
                    ...
                    match tg_syscall::handle(Caller { entity: 0, flow: 0 }, id, args) {
                        Ret::Done(ret) => match id {
                            Id::EXIT => {
                                lec9_lab6::emit_process_exit(task.pid.get_usize(), ret);
                ...
                // ─── 其他异常/中断：杀死进程 ───
                e => {
                    let sepc = task.context.context.pc();
                    let cause_str = format_exception_str(e);
                    lec9_lab6::emit_exception_kill(
                        task.pid.get_usize(),
                        &cause_str,
                        stval::read(),
                        sepc,
                    ...
                    println!("\n===== Exception Backtrace (kernel) =====");
                    stackwalk::print_backtrace();
                    println!("===== End Exception Backtrace =====\n");
...
#[cfg(feature = "show-demos")]
#[inline(never)]
fn bt_depth2(x: usize) {
    bt_depth3(x, x + 1);
}

#[cfg(feature = "show-demos")]
#[inline(never)]
fn bt_depth3(a: usize, b: usize) {
...
#[inline(never)]
fn trigger_error() {
    panic!("deliberate panic to demonstrate error backtrace");
}

fn format_exception_str(trap: scause::Trap) -> String {
    format!("{:?}", trap)
}
    ...
    impl IO for SyscallContext {
        ...
        fn write(&self, _caller: Caller, fd: usize, buf: usize, count: usize) -> isize {
                ...
                .translate::<u8>(VAddr::new(buf), READABLE)
            {
                if fd == STDOUT || fd == STDDEBUG {
                    ...
                    lec9_lab6::emit_write_result(current.pid.get_usize(), fd, count, ret);
                    ret
                    ...
                    if file.writable() {
                        ...
                        let ret = file.write(UserBuffer::new(v)) as isize;
                        lec9_lab6::emit_write_result(current.pid.get_usize(), fd, count, ret);
                        ret
                    } else {
                        log::error!("file not writable");
                        lec9_lab6::emit_write_result(current.pid.get_usize(), fd, count, -1);
                        -1
                    }
                } else {
        ...
        fn read(&self, _caller: Caller, fd: usize, buf: usize, count: usize) -> isize {
                ...
                .translate::<u8>(VAddr::new(buf), WRITEABLE)
            {
                if fd == STDIN {
                    ...
                    let ret = count as isize;
                    lec9_lab6::emit_read_result(current.pid.get_usize(), fd, count, ret);
                    ret
                } else if let Some(file) = &current.fd_table[fd] {
                    // 普通文件：通过文件句柄读取
                    let file = file.lock();
                    if file.readable() {
                        let mut v: Vec<&'static mut [u8]> = Vec::new();
                        unsafe { v.push(core::slice::from_raw_parts_mut(ptr.as_ptr(), count)) };
                        let ret = file.read(UserBuffer::new(v)) as isize;
                        lec9_lab6::emit_read_result(current.pid.get_usize(), fd, count, ret);
                        ret
                    } else {
                        log::error!("file not readable");
                        lec9_lab6::emit_read_result(current.pid.get_usize(), fd, count, -1);
                        -1
                    }
                } else {
                    log::error!("unsupported fd: {fd}");
        ...
        fn open(&self, _caller: Caller, path: usize, flags: usize) -> isize {
            ...
            if let Some(ptr) = current.address_space.translate(VAddr::new(path), READABLE) {
                ...
                let oflags = OpenFlags::from_bits(flags as u32).unwrap();
                if let Some(fd) = FS.open(string.as_str(), oflags) {
                    let new_fd = current.fd_table.len();
                    current.fd_table.push(Some(Mutex::new(fd.as_ref().clone())));
                    lec9_lab6::emit_open_result(
        ...
        fn close(&self, _caller: Caller, fd: usize) -> isize {
            ...
            lec9_lab6::emit_close(current.pid.get_usize(), fd);
    ...
    impl Process for SyscallContext {
        ...
        fn fork(&self, _caller: Caller) -> isize {
            ...
            *context.a_mut(0) = 0 as _;
            ...
            lec9_lab6::emit_fork(parent_pid.get_usize(), pid.get_usize());
        ...
        fn exec(&self, _caller: Caller, path: usize, count: usize) -> isize {
            ...
            if let Some(fd) = FS.open(name, OpenFlags::RDONLY) {
                lec9_lab6::emit_exec_load(current.pid.get_usize(), name);
        ...
        fn sbrk(&self, _caller: Caller, size: i32) -> isize {
            ...
            if let Some(old_brk) = current.change_program_brk(size as isize) {
                lec9_lab6::emit_sbrk(current.pid.get_usize(), size, old_brk);
                old_brk as isize
            } else {
                -1
            }
