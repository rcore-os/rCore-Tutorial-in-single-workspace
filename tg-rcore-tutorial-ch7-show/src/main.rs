//! # 第七章（ch7-show）：进程间通信与信号 — 教学展示版
//!
//! 在 `tg-rcore-tutorial-ch7` 基础上增加：
//! **lec10 可观测输出**、**源码级 backtrace（含参数）**、**GDB+QEMU 管道/信号演示脚本**（见 `scripts/`、`gdb/`）。
//!
...
extern "C" fn rust_main() -> ! {
    ...
    let root_ppn = kernel_space(layout, MEMORY, portal_ptr as _);
    #[cfg(target_arch = "riscv64")]
    lec10_lab7::emit_kernel_space_created(root_ppn, (8usize << 60) | root_ppn);
    // 步骤 6：初始化异界传送门
    ...
    if let Some(process) = Process::from_elf(ElfFile::new(initproc.as_slice()).unwrap()) {
        ...
        #[cfg(target_arch = "riscv64")]
        lec10_lab7::emit_init_observables(MEMORY);
        #[cfg(target_arch = "riscv64")]
        {
            println!("\n===== ch7-show: Normal Backtrace Demo =====");
            bt_depth1();
        ...
        if let Some(task) = unsafe { (*processor).find_next() } {
            ...
            match scause::read().cause() {
                ...
                scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                    ...
                    #[cfg(target_arch = "riscv64")]
                    lec10_lab7::emit_before_handle_signals(task.pid.get_usize());
                    match task.signal.handle_signals(ctx) {
                        // 收到终止信号（如 SIGKILL），进程应该退出
                        SignalResult::ProcessKilled(exit_code) => {
                            #[cfg(target_arch = "riscv64")]
                            lec10_lab7::emit_signal_terminates(task.pid.get_usize(), exit_code);
                            unsafe { (*processor).make_current_exited(exit_code as _) }
                        }
                        // 未被终止，继续处理系统调用返回值
                        _ => {
                            #[cfg(target_arch = "riscv64")]
                            lec10_lab7::emit_signal_check_done(task.pid.get_usize());
                            match syscall_ret {
                ...
                e => {
                    ...
                    let cause_str = alloc::format!("{e:?}");
                    #[cfg(target_arch = "riscv64")]
                    lec10_lab7::emit_exception_kill(
                        task.pid.get_usize(),
                        &cause_str,
                        stval::read(),
                        sepc,
...
fn kernel_space(layout: tg_linker::KernelLayout, memory: usize, portal: usize) -> usize {
    ...
    unsafe { satp::set(satp::Mode::Sv39, 0, root) };
...
mod impls {
    ...
    impl IO for SyscallContext {
        ...
        fn pipe(&self, _caller: Caller, pipe: usize) -> isize {
            ...
            #[cfg(target_arch = "riscv64")]
            lec10_lab7::emit_pipe_created(current.pid.get_usize(), read_fd, write_fd);
            0
        }
    }
