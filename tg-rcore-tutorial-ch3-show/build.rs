//! 构建脚本：
//!
//! 1. 生成自定义链接脚本（含 HEAP 段）
//! 2. 编译并嵌入用户程序
//! 3. 从上一次构建产物中提取 ELF 符号 + DWARF 行号/形参信息，
//!    生成 `func_syms_generated.rs` 供运行时 backtrace 使用
...
fn main() {
    ...
    if target_arch == "riscv64" {
        ...
        let triple =
            env::var("TARGET").unwrap_or_else(|_| "riscv64gc-unknown-none-elf".into());
        ...
        extract_symbols_from_prev_build();
    }
}

// ===== Linker script (NOBIOS + HEAP) =====
...
fn write_linker() {
    ...
    fs::write(&ld, LINKER_SCRIPT).unwrap_or_else(|err| {
        panic!("failed to write linker script to {}: {}", ld.display(), err)
    });
    println!("cargo:rustc-link-arg=-T{}", ld.display());
}

// ===== User app building (unchanged from ch3) =====
...
// ===== ELF / DWARF symbol extraction (from ch1-show build.rs) =====

const TEXT_BASE: u64 = 0x80200000;
const TEXT_LIMIT: u64 = 0x81000000;

fn extract_symbols_from_prev_build() {
    let out_dir = PathBuf::from(env::var_os("OUT_DIR").unwrap());
    ...
    write_generated_rs(&syms, &dwarf, &out_dir.join("func_syms_generated.rs"));
}
...
// ===== DWARF line table + parameter extraction =====
...
fn extract_param_locs(dwarf: &addr2line::gimli::Dwarf<DwarfSlice<'_>>) -> Vec<ParamLoc> {
    ...
    result.sort_by(|a, b| {
        a.func_addr
            .cmp(&b.func_addr)
            .then(a.fbreg_offset.cmp(&b.fbreg_offset))
    });
    result
}
...
fn write_generated_rs(syms: &[(u64, u64, String)], dwarf: &DwarfInfo, out: &std::path::Path) {
    use std::fmt::Write;

    let mut c = String::with_capacity(
        syms.len() * 100
            + dwarf.line_table.len() * 30
            + dwarf.file_paths.len() * 60
            + 1024,
    );
