//! 裸机堆：固定区域 bump 分配器，供 `alloc` / `addr2line` 使用。
//!
...
/// 与 `build.rs` 中 `HEAP_SIZE` 相同（`0x1000000`，addr2line 解析 DWARF 可能需数 MiB）。
...
// 由链接脚本 `PROVIDE(__heap_start)` / `__heap_end` 提供。
unsafe extern "C" {
    safe static __heap_start: u8;
    safe static __heap_end: u8;
}

/// 必须在任何堆分配之前调用一次（例如在 `rust_main` 开头）。
pub fn init() {
