...
- **动态执行行为**：借助帧指针链打印**调用栈**（与 [`axbacktrace`](https://github.com/Starry-OS/axbacktrace) 同一套 RISC-V 回溯思路），理解「程序在跑什么、从哪一层调用进来」
...
- [ ] （与**第二讲**衔接）运行 `bash test_lec2_lab1.sh`，串口输出中出现全部 `[LEC2-LAB1] kp=...` 可 grep 标签，并能对照下表口述每条标签对应的课堂概念
- [ ] 能结合串口 `[BACKTRACE]` 输出，说明**调用栈**与**控制流嵌套调用**（`bt_depth3`→…→`rust_main`）的对应关系；可选：用 `llvm-addr2line` / `riscv64-unknown-elf-addr2line` 将 `ra` 解析到函数名与行号
...
本仓库在 `Hello, world!` 之后打印一组以 `[LEC2-LAB1]` 开头的**稳定标签行**，用于把 [`knowledge-points.md`](../../discussions/lec-vs-lab/knowledge-points.md) 中**第 2 讲**的表述，落到「命令 + 子串」证据链（与 `discussions/lec-vs-lab/01-lec2-ch1-ir.md` 的 `observables` / `contract` 字段同构）。

| 第二讲知识点（大纲摘要） | Lab1 中的可观测证据 | 源码与自测 |
|--------------------------|---------------------|------------|
| 实验整体设计思路与各章能力目标 | `kp=curriculum` 行（ch1→后续章节递进） | `src/lec2_lab1.rs`；`grep kp=curriculum` |
| 函数调用 / RISC-V 调用约定；与 syscall 的差异（预告） | `kp=callconv` 行；`result=0x30`；提示用反汇编看 a0/a1 | `lec2_demo_add`；`riscv64-unknown-elf-objdump -d` 或 `llvm-objdump -d` |
| 编译器 / 硬件 / OS 共识（ABI、目标文件、链接） | `kp=compile_abi` 含 `target=...` 与 `panic=abort no_std` | `.cargo/config.toml`、`Cargo.toml`、`build.rs` |
| 加电启动链：QEMU、内存布局、firmware→S 态入口；SBI 调用 | `kp=boot_chain`（nobios 参数模板）；`kp=mem_layout`（M/S 基址） | README「裸机启动流程」；`grep kp=boot_chain` |
| ELF、`linker.ld`、程序内存布局 | `kp=mem_layout`；链接脚本由 `build.rs` 生成 | `build.rs` 中 `LINKER_SCRIPT`；`env!("LAB_*")` 注入 |
| LibOS：裸机初始化、自建栈、最小执行环境 | `kp=libos_stack sp=0x...`；`kp=control_flow` | `src/main.rs` 的 `_start`；`grep kp=libos_stack` |
| SBI 与特权级（对照将来 syscall） | `kp=sbi_vs_syscall` | `rust_main` 中 `console_putchar`/`shutdown`；阅读 `tg-rcore-tutorial-sbi` |
| 异常路径 / panic 契约（不变量） | `kp=panic_contract` | `panic_handler` 中 `shutdown(true)`；可自编 `panic!()` 实验 |

**自测命令：**

- 基线（兼容旧判据）：`bash test.sh`
- 第二讲全量观测：`bash test_lec2_lab1.sh`
