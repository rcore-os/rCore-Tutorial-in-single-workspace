...
fn main() {
    ...
    if target_arch == "riscv64" {
        ...
        println!("cargo:rustc-env=LAB_M_BASE=0");
        println!("cargo:rustc-env=LAB_S_BASE=0");
        println!("cargo:rustc-env=LAB_TARGET_TRIPLE=host-non-riscv");
    }
...
// ===== ELF / DWARF symbol extraction（与 ch4-show 一致，供运行时回溯解析） =====
