...
//! ## ch5-show 扩展
//!
//! - **`lec7_lab5`**：串口输出 `[LEC7-LAB5]` 知识点锚点；
//! - **`stackwalk` / `symtab_resolve`**：帧指针回溯 + 编译期嵌入符号，打印带参数的源码级栈；
//! - **`scripts/` + `gdb/`**：QEMU + `riscv-none-elf-gdb-py3` 观察调度与 trap。
...
extern "C" fn rust_main() -> ! {
    ...
    if let Some(process) = Process::from_elf(ElfFile::new(initproc_data).unwrap()) {
        let init_pid = process.pid.get_usize();
        let init_entry = process.context.context.pc();
        let init_satp = process.context.satp;
        // 初始化进程管理器并添加 initproc
        PROCESSOR.get_mut().set_manager(ProcManager::new());
        PROCESSOR
            .get_mut()
            .add(process.pid, process, ProcId::from_usize(usize::MAX));
        lec7_lab5::emit_initproc_scheduled(init_pid, init_entry, init_satp);
    }

    lec7_lab5::emit_init_observables(APPS.len(), MEMORY);

    println!("\n===== Normal Backtrace Demo (ch5-show) =====");
    ch5_show_bt_depth1();
    println!("===== End Normal Backtrace Demo =====\n");

    // ─── 主调度循环 ───
    // 不断从进程管理器中取出就绪进程执行，直到所有进程结束
    let mut prev_running_pid: Option<usize> = None;
    loop {
        let processor: *mut PManager<Process, ProcManager> = PROCESSOR.get_mut() as *mut _;
        if let Some(task) = unsafe { (*processor).find_next() } {
            let run_pid = task.pid.get_usize();
            let run_satp = task.context.satp;
            let run_pc = task.context.context.pc();

            if prev_running_pid != Some(run_pid) {
                if let Some(from) = prev_running_pid {
                    lec7_lab5::emit_context_switch_hint(from, run_pid, run_satp);
                }
                lec7_lab5::emit_portal_enter_user(run_pid, run_pc, run_satp);
            }
            lec7_lab5::emit_schedule_pick(run_pid, run_pc, run_satp);
            prev_running_pid = Some(run_pid);
            ...
            match scause::read().cause() {
                ...
                scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                    ...
                    let trap_pid = task.pid.get_usize();
                    let sepc_at_trap = ctx.pc();
                    let syscall_raw = ctx.a(7);
                    let a0_at_trap = ctx.a(0);
                    lec7_lab5::emit_syscall_trap(trap_pid, syscall_raw, a0_at_trap, sepc_at_trap);
                    ...
                    match tg_syscall::handle(Caller { entity: 0, flow: 0 }, id, args) {
                        Ret::Done(ret) => match id {
                            ...
                            Id::EXIT => {
                                lec7_lab5::emit_process_exit(trap_pid, ret as usize);
                                unsafe { (*processor).make_current_exited(ret) };
                                prev_running_pid = None;
                            }
                            _ => {
                                if id == Id::CLONE {
                                    lec7_lab5::emit_fork_return_parent(trap_pid, ret);
                                }
                                if id == Id::SCHED_YIELD {
                                    lec7_lab5::emit_task_suspended(trap_pid);
                                }
                ...
                e => {
                    ...
                    let cause_str = format!("{:?}", e);
                    lec7_lab5::emit_exception_kill(trap_pid, cause_str.as_str(), stval::read(), sepc);
    ...
    println!("\n===== Panic Backtrace Demo (ch5-show) =====");
    ch5_show_trigger_panic_for_bt();
}

#[inline(never)]
fn ch5_show_bt_depth1() {
    ch5_show_bt_depth2(42);
}

#[inline(never)]
fn ch5_show_bt_depth2(x: usize) {
    ch5_show_bt_depth3(x, x + 1);
}

#[inline(never)]
fn ch5_show_bt_depth3(a: usize, b: usize) {
    core::hint::black_box((a, b));
    stackwalk::print_backtrace();
}

#[inline(never)]
fn ch5_show_trigger_panic_for_bt() -> ! {
    panic!("deliberate panic to demonstrate error-path backtrace (ch5-show)");
}
...
fn kernel_space(layout: tg_linker::KernelLayout, memory: usize, portal: usize) {
    ...
    let root = space.root_ppn().val();
    lec7_lab5::emit_kernel_space_created(root, (8usize << 60) | root);
    // 保存内核地址空间到全局变量
...
mod impls {
    use crate::{
        build_flags, lec7_lab5, process::Process as ProcStruct, processor::ProcManager, Sv39, APPS,
        PROCESSOR,
    };
        ...
        fn exec(&self, _caller: Caller, path: usize, count: usize) -> isize {
            ...
            current
                ...
                .map_or_else(
                    ...
                    |data| {
                        let pid = current.pid.get_usize();
                        current.exec(data);
                        lec7_lab5::emit_exec_success(pid);
                        0
                    },
                )
        }
